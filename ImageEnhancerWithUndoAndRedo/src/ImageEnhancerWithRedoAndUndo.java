 ...
 * by Zhen Xu for CSE 373 Assignment 2, Winter, 2016.
...
public class ImageEnhancerWithRedoAndUndo extends Component implements ActionListener {
    ...
    public ImageEnhancerWithRedoAndUndo() {
    	createMenu();
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	// if undo is clicked, pop the undo stack
    	// meanwhile, push the current image into redo stack
    	if (e.getSource()==undoItem) {
    		biFiltered = undo.pop();
    		redo.push(copy(biWorking));
    	}	
    	// if click on the menu bar is performed, push the current image into undo stack
    	else {
    		undo.push(copy(biWorking));
    	}
    	
    	if (e.getSource()==redoItem) {biFiltered = redo.pop();}
    	// perform redo only when undo has been performed
    	// both are disabled when stack is all clear
    	if (undo.isEmpty()) { 
    		undoItem.setEnabled(false); 
    	}
    	else { 
    ...
    // this copy method as an buffer for biworking image so as the current image is not influenced anyway
    ...
    private ImageEnhancerWithRedoAndUndo image_enhancer_instance;
    public ImageEnhancerWithRedoAndUndo getImageEnhancer() { // For use by the autograder
    		if(image_enhancer_instance == null) {
    			image_enhancer_instance = new ImageEnhancerWithRedoAndUndo();
    		}
    ...
    public static void main(String s[]) {
    	new ImageEnhancerWithRedoAndUndo().run(); // Called from below, and by the autograder.
    }
    
    public void run() {
        JFrame f = new JFrame("ImageEnhancer with Undo AND Redo by Zhen Xu"); // Students should update this.
